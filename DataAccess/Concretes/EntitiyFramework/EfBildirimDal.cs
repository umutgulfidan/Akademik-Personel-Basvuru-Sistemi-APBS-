...
namespace DataAccess.Concretes.EntitiyFramework
{
    ...
    {
        public async Task<List<Bildirim>> GetAllWithPaginating(AdminBildirimQueryDto query)
        {
        ...
        public async Task<List<Bildirim>> GetMyPaginatedNotifications(int userId,UserBildirimQueryDto query)
        {
            using var context = new Context();
            // Veritabanı sorgusunu başlatıyoruz
            var bildirimsQuery = context.Bildirimler.AsNoTracking().AsQueryable();

            bildirimsQuery = bildirimsQuery.Where(x => x.KullaniciId == userId);
